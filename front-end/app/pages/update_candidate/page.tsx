...
export default function UpdateCandidate() {
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            try {
                const username = localStorage.getItem('update_can');
                const token = localStorage.getItem('token');

                if (username && token) {
                    const response = await axios.post(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/sys_admin/search_candidates`, {
                        username: username
                    }, {
                        headers: {
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            await axios.patch(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/sys_admin/update_candidates`, {
                username: username,
                name: formData.name,
            ...
            router.push('../pages/view_candidates');
        } catch (error) {
            console.error('Error:', error);
        }
    };
    ...
    return (
        <div>
            <br />
            <h1 className="text-4xl font-bold text-red-900 pl-4">Update Candidate</h1>
            <br />
            <div className="flex justify-center items-center">
                <div className="p-8 rounded shadow-md bg-white bg-opacity-75">
                    <form onSubmit={handleSubmit}>
                        ...
                        {errorPName && <div className="text-red-900 font-bold mb-4">{errorPName}</div>}
                        <div className="mb-4">
                            <label className="block mb-1 text-black">Position:</label>
                            <input
                                type="text"
                                id="position"
                                name="position"
                        ...
                        {errorPosition && <div className="text-red-900 font-bold mb-4">{errorPosition}</div>}
                        <div className="mb-4">
